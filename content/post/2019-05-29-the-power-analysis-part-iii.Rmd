---
title: Power Analysis by Data Simulation - Part III
...
Before I even start with this topic, I want to point out that I took much of the information I present here from Jonas Kristoffer Lindeløv's __fantastic__ blog post ["Common statistical tests are linear models (or: how to teach stats)"](https://lindeloev.github.io/tests-as-linear/). 
...
First of all, here is a quick reminder on the linear model with a quick example:

$$
y_i = \beta_0 + \beta_1x_i
$$

The above formula states that each value of y (the little i means "each" in this case) can be described by adding a value $\beta_0$ and the product of $beta_0 \times x$.
...
However, if we reconsider the problem what we can also say that whether group membership (whether you belong to group1 or group2; the $x$ variable in this case) significantly _predicts_ the person's score (y in this case).
...
Just like $\beta_0$, our new mean-difference parameter $\beta_1$ is an unkown that depends on our simulated means and will be estimated by the model.
...
Again (cannot mention this enough) make sure you head to Jonas Kristoffer Lindeløv's [blog post](https://lindeloev.github.io/tests-as-linear/) if you want to see how to do this or if you want to read more on this topic.
...
Moreover, we are interested in whether the attitude that people report is different after seing a message that regards the consequences on other people (e.g. smoking can harm your loved ones) as compared to yourself (smoking can cause cancer).
...
Lets assume that we expect that people's attitude will be more negative after seing a visual rather than text message if the focus is internal (i.e. the message is about yourself) because it is difficult to imagine that oneself would get cancer after reading a text but seing a picture might cause fear regardless.
For the external focus on the other hand, we expect a more negative attitude after reading a text as compared to seing a picture, as it has more impact on attitude to imagine a loved one get hurt than seeing a stranger in a picture suffering from the consequences of second-hand smoking.
...
A first good step is to visualize some rough means that show the desired behaviour that we described in words earlier and see where we are going, starting with the main-effects.
...
This a good point for a short reflection: You might already be overwhelmed how complicated power-anaylsis by simulation already is with these still reletively simple designs.
...
Our interaction-hypothesis is mainly interested in wether the difference in scores between the visual and the text messages when comparing the internal focus group is the same as the difference for the external groups.
As this is clearly not the case in this example, we are good to go and test the interaction hypothesis.
However, if we clearly are intrested in the simple slopes and have hypotheses about them, we _can_ put it in our simulation, and that of course urges us to think some more but it is a _great_ tool to see whether we actually thought about everything, as this might be a point that would have slipped our attention entirely when running a classical power-anaylsis and pre-registering the overall interaction only.
...
For instance, we can assume in this case that all groups might actually have a more negative attitude towards smoking after seing the text or visual message no matter whether they were externally and internally (ideally we would of course use a pre-post test design but lets keep that for later).
Thus, we could for example assume that (having no better data on this) people are on average indifferent about smoking, thefore assuming the population mean is 50.
...
however, as we want to keep the rank-order of the bars the same (i.e. order of difference-magintudes between the groups should stay the same as above) we will start with the highest graph here and adjust it to a position where it would actually make sense compared to the population mean.
...
Next, when looking at the TI group, I thinkt hat this group might decrease more strongly compared to the other group (it is internal), so the new mean after reading texts about yourself might be 45 in this case.

The next group is the TE group which should be lower than the other two groups before. However given that it is also an external group, and I expect internal messages to work better, it is probably not much stronger than the TI group, so I wil assume the mean is 43.
...
If we want to test whether the effect of the interaction, for instance is signficant, we can conduct an ANOVA for the interaction to see how the model _with_ the interaction compares to a model _without_ the interaction, which we do in the following code-block by fitting a model that does not have the interaction in their and compare it with the `anova` function.
...
An obvious advantage of the `aov` function is that we do not have to specify nested models but it is also sort of a black-box approach, while in the linear models, we get estimated effects and can see whether the model does actually do what we want it to do more accurately.
...
We see immideately that the estimates changed somewhat.
...
Why would we use this coding-scheme of setup to the linear model instead of the other one? 
...
This is why the results of the last anova do not show the behaviour that we simulated.
...
The only thing that we still need to do is specify our criteria.
Imagine that this time we are highered by the government and are asked to evaluate which of the health-warnings on cigarrette packages would work best in which of the situations that we investigate.
...
Now we have everything that we need to set up the code-chunk below to run a power-analysis similar to the ones we have been running previously, just this time with an anova.
If we would again try out every sample-size, this would take a very long time until we reach our power criterion and we would probably still be sitting here tommorrow or the day after.
In these cases, it makes sense to run a two-stage power analysis in which we, in the first stage, try out various sample-sizes with a very low resultion (e.g. n = 10, n = 100, n = 200) etc. to get a rough idea of where we can expect our loop to end.
...
while(power_at_n[i-1] < power_crit){
  for(sim in 1:n_sims){
    ...
    p_vals[sim] <- anova(lm_int, lm_null)$`Pr(>F)`[2]
  }
    print(n)
...
No we now that we can have a closer look at sample-sizes between 800 and 900.
Probably, at this sample-size it does not really matter whether we calculate +/- 10 participants, so we can start at 800 and increase in steps of 10 and instead increase the simulation-number per sample-size to 10,000 to get even more precise results, as this is a rather important power-analysis impact-wise.
Afterwards, if we _really_ wanted to we could of course do the same between the last two 10-people cutoffs and increase the sample-size by 1 to reach an exact sample-size.
...
Later, when we work with mixed models in part IV, we will see how we can run such a simulation on multiple cores to solve this problem to at least some extent.
...
Note how close this is to the 900 from our low-resolution simulation above.
...
Whether this ~0.6% is a meaningful amount of added variance explained to begin with and whether it is worth spending money in order to test about 3,500 people without even knowing what this attitude measure in this case even tells us is of course another question and one not to be answered by power-analysis (probably the answer would be that it is not so important though).
...
Again, knowing how to respecify an ANOVA as a linear model is helpful in this case if we have both, factorial and numerical predictors.
...
Let us say we assume this to be 36 cigarretes a day (about 2 packages).
Now if we want a person that smokes 0 cigarretes to get the full 7 points and a person who smokes 36 cigarettes to get 0 percent of the effect, we can rewrite the effect like this
...
For the smoking population we will choose a _poisson_ distribution, as it is often a good representation of count-data like this.
