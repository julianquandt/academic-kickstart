...
authors: []
lastmod: '2020-01-16T11:44:32+01:00'
featured: no
image:
  caption: ''
  focal_point: ''
...
Now the story gets a bit technical I'm afraid, and gets us back to stats 101 where we might have learned about _probability mass function._ In this case we need the __binomial__ probability mass function.
...
<details><summary>__CLICK HERE FOR MORE ON BINOMIAL LIKELIHOOD__</summary>
<p>
...
In the example above we already used three concepts from frequentist statistical testing:
...
In a proper power analysis, all of these need to be specified in addition to an _alternative hypothesis_.

## Specifying the null-hypothesis.

Again, we will assume that a coin is fair and will produce heads with a probability of .50[^9].

## Specyfing the Alternative Hypothesis aka Effect Size.

To conduct a proper power-analysis, it is important that we specify a concrete _alternative hypothesis_ (or effect size).
If we do know have a hypothesis about the effect size, we can _by definition_ not define the power to investigate this hypothesis. 
Makes sense, right?
In most software packages, you will use effect-size estimates like Cohen's d or f or other _standardized effect sizes_.[^6] 

Here, we will follow a different approach by trying to specify our hypothesis, the expected effect size, on the _raw_ scale. 
...
Furthermore, as it does not deem you a good idea to get into a fight with Harvey by incorrectly accusing him of using an unfair coin, you want the chance of this happening (i.e. the alpha-level) to be very low at only 0.1 percent (1 in a 1000 cases)[^8].

## Specyfing the desired power
...
Thus, when tossing a coin 1908 times, in 90% of the cases we will be able to tell it's biased and can confidently confront Harvey like shown in Figure 5.
...
[^6]:
Software like G*Power often uses different standardized effect sizes for different anaylses. 
...
[^7]:
For those unfamiliar with this reference: First, consider watching Dark Knight, it's a great movie. Second, this character (Harvey Dent, a former state lawyer) decides whether or not to sentence (kill) people by tossing a coin.
...
Maybe, maybe not, but we can define a _smallest effect size of interest_ and use _Equivalance testing_ in which the null-hypothesis is a certain range of small deviations from the actual point of no-difference in which we say that the effect is too small to care about. 
...
[^13]:
Technically the vector contains FALSE instead of 0 and TRUE instead of 1. However, in R, and many other programming languages, the two are interchangable, allowing us to calculate the mean in the same way.
